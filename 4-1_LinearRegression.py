...
for col in data.columns:
    if data[col].dtype == "object":  # 문자열 타입인 경우
        data[col] = data[col].str.replace(",", "").astype("float", errors="ignore")

# 결측치 처리 (평균값으로 대체)
data.fillna(data.mean(numeric_only=True), inplace=True)

# 2015년~2023년 데이터로 학습, 2024년 데이터로 테스트
...
X_train = train_data.drop(columns=['분기', '종가', '년도'])
y_test = test_data['종가']
X_test = test_data.drop(columns=['분기', '종가', '년도'])
